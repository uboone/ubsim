...
#include "TTimeStamp.h"
  ...
  namespace CTMDataOFFAna {  
    ...
    public:
     ...
     explicit CTMDataOFFAna(fhicl::ParameterSet const &pset);
     virtual ~CTMDataOFFAna();                        
   ...
   fDetector(lar::providerFrom<detinfo::DetectorPropertiesService>())
  {
    ...
    //-----------------------------------------------------------------------
    // Destructor
    CTMDataOFFAna::~CTMDataOFFAna() 
   {
    }
   //-----------------------------------------------------------------------
   void CTMDataOFFAna::ClearLocalData()
   {
...
*/    fDataTreeFinalSel->Branch("_fNrecomult",&_fNrecomult,"_fNrecomult/I");
   ...
   //-----------------------------------------------------------------------
   void CTMDataOFFAna::endJob()
  {

  }
  
   //-----------------------------------------------------------------------
   void CTMDataOFFAna::beginRun(const art::Run& run)
  {
    
  }
    //-----------------------------------------------------------------------
   void CTMDataOFFAna::reconfigure(fhicl::ParameterSet const& pset)
   {
    ...
    fBeamMax                 = pset.get<double>      ("BeamMax", 5.25);
...
double CTMDataOFFAna::FlashTrackDist(double flash, double start, double end) const
{
    if(end >= start) {
   ...
   void CTMDataOFFAna::analyze(art::Event const& evt) 
   {
    _frun = evt.run();
    _fsubrun = evt.subRun();
...
///////////////////////////////////////////////////////////////////////////////
   ...
   ClearLocalData();
   return;
  } // end CTMDataOFFAna::analyze()
   	
   DEFINE_ART_MODULE(CTMDataOFFAna)
