...
SnAssemblerDriver::SnAssemblerDriver(fhicl::ParameterSet const &pset,
                      art::ProductRegistryHelper &helper,
                      art::SourceHelper const &pm)
                        : fSourceHelper(pm)
{
 
  mf::LogError("SnAssemblerDriver") << "SnAssemblerDriver constructor"; 
  mf::LogInfo("SnAssemblerDriver") << pset.to_string();
  fAssembler.reset(new SnAssembler(pset));

  fRequest.run    = pset.get< unsigned int  >("run");
  fRequest.subrun = pset.get< unsigned int  >("subrun");
  fRequest.frame =  pset.get< unsigned int   >("frame");
...
void SnAssemblerDriver::closeCurrentFile()
{
  mf::LogError("SnAssemblerDriver") << "SnAssemblerDriver closeCurrentFile()"; 
  
}

void SnAssemblerDriver::readFile(std::string const &name,
                                  art::FileBlock* &fb)              
{
  mf::LogError("SnAssemblerDriver") << "SnAssemblerDriver readFile()"; 
  mf::LogInfo("SnAssemblerDriver") << "Requested name: " << name; 
  // Fill and return a new Fileblock.
  fb = new art::FileBlock(art::FileFormatVersion(1, "Supernova ubdaq file codename: teflon"),
                          name);
 
}


bool SnAssemblerDriver::readNext(
              ...
              art::EventPrincipal* &outE)
{
  ...
  mf::LogError("SnAssemblerDriver") << "SnAssemblerDriver readNext()"; 


  // Get the event.
  std::shared_ptr<gov::fnal::uboone::datatypes::ub_EventRecord> event = fAssembler->assemble(fRequest);
