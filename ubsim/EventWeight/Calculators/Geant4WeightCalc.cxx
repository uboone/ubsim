 ...
 * Updated by C. Thorpe <c.thorpe@lancaster.ac.uk>,
...
//C Thorpe: Add multidimensional gaussian
#include "TRandom2.h"
...
namespace evwgh {
	...
	class Geant4WeightCalc : public WeightCalc {
		...
		private:
			...
			std::string fCovarianceFile;
			std::vector<std::vector<double>> GetUniversesWithCovariance(TMatrixDSym cov , std::vector<double> centralValues, int Nsims);

			//uniform RNG - used to make multivariate RNG
			CLHEP::RandFlat* fUniformRandom;
			bool fUseFitCentralValue=false; //if true, will use central values from fit
			...
			CLHEP::HepRandomEngine& engine)
	{
		...
		//if using covariance matrix, open cov matrix file
		if(mode == "covariance"){
			fCovarianceFile = pset.get<std::string>("CovarianceFile");
			fUseFitCentralValue = pset.get<bool>("UseFitCentralValue");
		}
		...
		fUniformRandom = new CLHEP::RandFlat(engine);
		...
		if (fPdg==2212 || fPdg == 2112) totalonly = true;
		...
		if (mode=="pm1sigma"){
			...
			//count num of positive and negative rns generated
			...
			for (unsigned j=0; j<fNsims; j++){
				...
				for (size_t i_parset=0; i_parset<n_parsets; ++i_parset){
					double r = fGaussRandom->fire(0,1.0); //CT: Correction made here  
			...
			std::cout << "Using covariance matrix from file: " << fCovarianceFile <<  std::endl;

			//open file containing covariance matrix object
			TFile CovarianceFile(fCovarianceFile.c_str() , "OPEN");

			//get the covariance matrix object and names of the parameters used in it
			//2d histogram in the file contains the covariance matrix
			TH2D *CovHist;
			CovarianceFile.GetObject("FitCovHist",CovHist);

			int nBins = CovHist->GetXaxis()->GetNbins();

			TH1D *parsHist=nullptr; //histogram of fit param central values (use if fUseFitCentralValue=true)

			if(fUseFitCentralValue)	CovarianceFile.GetObject("parsHist",parsHist);

			//search input parameter list for which things you want to reweight, search cov matrix for corresponding bins

			//form map between parameters and their covariances
			...
			std::vector<double> FitParSigmas;
			std::map<std::string, double> theNominals;
			std::map< std::pair<std::string,std::string>,double>theCovElements; //links pair of string to elem of cov matrix

			//store param names and nominal vals

			if(fDebug) std::cout << "Parameter Names:" << std::endl;
			...
			for (size_t i_parset=0; i_parset<n_parsets; ++i_parset){
				...
				//check if parameter is to be taken from covariance matrix, if it is and
				//it is missing from covariance matrix, throw exception


				int covBinNo = CovHist->GetXaxis()->FindBin(theName.c_str());
				bool useCovariance = theSet.get<bool>("UseCovariance",true);			

				//if the corresponding bin the covariance histogram isn't found, Find bin should return underflow/overflow bin num
				if( useCovariance &&  (covBinNo <= 0 || covBinNo >= nBins+1 )){
					std::string message = "Parameter " + theName  + " not found in covariance matrix, exiting";
					throw std::invalid_argument(message);
				}

				//if not using central values from fit, or parameter does not exist in pars hist, use nominal from params file
				if(!fUseFitCentralValue || !(CovHist->GetXaxis()->FindBin(FitParNames.at(i_parset).c_str()) < nBins+1 && CovHist->GetXaxis()->FindBin(FitParNames.at(i_parset).c_str()) > 0) ) 
					theNominal = theSet.get<double>("Nominal",1.);		

				else theNominal = parsHist->GetBinContent(parsHist->GetXaxis()->FindBin(theName.c_str()));


				if(fDebug) std::cout << theName << "  " << theNominal << std::endl;		
			...
			//store list of names of params and their covariance elements
			std::map< std::pair<std::string,std::string> , double > namesAndCovs;

			//covaiance matrix object
			TMatrixDSym cov(FitParNames.size());


			for(size_t i_parset1=0;i_parset1<n_parsets;++i_parset1){

				for(size_t i_parset2=0;i_parset2<n_parsets;++i_parset2){


					int covBinNo1 = CovHist->GetXaxis()->FindBin(FitParNames.at(i_parset1).c_str());
					int covBinNo2 = CovHist->GetXaxis()->FindBin(FitParNames.at(i_parset2).c_str());


					//first check the parameters exist in the covariance matrix - if bin label does not exist FindBin will give underflow/overflow bin num
					if( covBinNo1 < nBins+1 && covBinNo2 <  nBins+1 && covBinNo1 > 0 && covBinNo2 > 0 ){ 

						namesAndCovs[std::make_pair( FitParNames.at(i_parset1),FitParNames.at(i_parset2) )] = CovHist->GetBinContent(covBinNo1,covBinNo2);


						//set value of cov matrix
						cov(i_parset1,i_parset2) = CovHist->GetBinContent(covBinNo1,covBinNo2);


					}//if parameter exists in covariance matrix

					//if parameter is not found in the cov matrix provided, still add to the covariance for the fit, but use the value of sigma from the params
					//file and set off off diagonal elements to zero
					else
					{
						//if not on diagonal, set to zero
						if(i_parset1 != i_parset2){
							namesAndCovs[std::make_pair( FitParNames.at(i_parset1),FitParNames.at(i_parset2) )] = 0.0;
							cov(i_parset1,i_parset2) = 0.0;

						}//if not on diagonal
						else {

							std::cout << "Parameter " << FitParNames.at(i_parset1) << " not found in covariance matrix, using sigma from config file instead" << std::endl;

							namesAndCovs[std::make_pair( FitParNames.at(i_parset1),FitParNames.at(i_parset2) )] = FitParSigmas.at(i_parset1)*FitParSigmas.at(i_parset1);

							cov(i_parset1,i_parset2) = FitParSigmas.at(i_parset1)*FitParSigmas.at(i_parset1);

						}//if on diagonal

					}//if parameter does not exist in covariance matrix

					if(fDebug) std::cout << "Covariance between: " <<  FitParNames.at(i_parset1) << " and " <<  FitParNames.at(i_parset2) << " is  " << cov(i_parset1,i_parset2) << std::endl; 

				}


			}
			//setup multidim gaussian

			std::vector<std::vector<double>>parsets = GetUniversesWithCovariance(cov,FitParNominals,fNsims);


			//largely functions same way as mutlisim mode, except parameter vals are drawn from MD gaussia


			for (unsigned j=0; j<fNsims; j++){
				// In each multisim universe, loop through all parameters. For each parameter, generate a new random number from Nominal-Sigma to Nominal+Sigma.
				std::map<std::string, double> tmp_vals;
				if(fDebug) std::cout << "Universe: " << j << "   ";

				//if using Fit central value, set the central values as universe 0

				for (size_t i_parset=0; i_parset<n_parsets; ++i_parset){				

					//get parameter values from multivariate gaussian and put them here
					//get generated i_parset th parameter from jth universe

					if(j != 0 || !fUseFitCentralValue)
						tmp_vals[FitParNames.at(i_parset)] = parsets.at(j).at(i_parset);
					else
						tmp_vals[FitParNames.at(i_parset)] = FitParNominals.at(i_parset);

					if(fDebug) std::cout << FitParNames.at(i_parset) << " " << parsets.at(j).at(i_parset) << "  ";
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	//     Generate parameter sets using covariance matrix and central values specified in params      //
	/////////////////////////////////////////////////////////////////////////////////////////////////////


	std::vector<std::vector<double>> Geant4WeightCalc::GetUniversesWithCovariance(TMatrixDSym cov , std::vector<double> centralValues, int Nsims){


		std::cout << "Making multivariate gaussian" << std::endl;

		//get correlation matrix (inverse of cov)

		TMatrixDSym corr = cov;
		corr.Invert(); //correlation matrix
		Double_t matrixDeterminant = cov.Determinant();

		if(fDebug) std::cout << "Det(Cov) = " << matrixDeterminant << std::endl;

		std::vector<std::vector<double>>parvals;

		//make sure sizes of cov and central value vector are compatible! if not throw exception
		if((size_t)cov.GetNcols() != centralValues.size()){
			std::cout << "CV and Covariance have different sizes!!!" << std::endl;
			std::string message = "Covariance matrix has " + std::to_string(cov.GetNcols()) + " cols, central value vector has " + std::to_string(centralValues.size()) + " elements";		
			throw std::invalid_argument(message);
		}

		
		//TODO: Might be sensible to add a "time out" condition here to prevent infinite loop
		//in case of bad config leading to every test universe being rejected or trying to make too many universes 
		int naccepted=0;
		while(naccepted < Nsims ){

			std::vector<double>vals;

			//generate some parameter values
			//for each parameter, generate from a uniform dist with limits	
			// [0.0 , CV + 3sigma] 

			for(Int_t i=0;i<cov.GetNcols();i++){

				vals.push_back(fUniformRandom->fire(0.0,centralValues.at(i)+3*sqrt(cov(i,i))));


			}

			//calculate value of gaussian at these vals

			double exp_arg=0;

			for(Int_t i=0;i<cov.GetNcols();i++){
				for(Int_t j=0;j<cov.GetNcols();j++){

					exp_arg += (vals.at(i) - centralValues.at(i))*corr(i,j)*(vals.at(j) - centralValues.at(j)); 

				}
			}


			//height of gaussian at vals
			double denominator = sqrt( pow(2*3.1416 , cov.GetNcols())*matrixDeterminant );
			double PD = exp(-0.5*exp_arg)/denominator;

			//height of Gaussian at central value is 1.0/denominator
			double value = fUniformRandom->fire(0,1.0/denominator);

			if(value < PD){
				if(fDebug) std::cout << "Prepared " << naccepted << " th Universe" << std::endl;
				naccepted++; 
				parvals.push_back(vals);
			}
	...
	std::vector<std::vector<double> >
		Geant4WeightCalc::GetWeight(art::Event& e) {
			...
			for (size_t itruth=0; itruth<truthParticles.size(); itruth++) {
				...
				//get list of MC Particles in this event
				auto const& mcparticles = truthParticles.at(itruth);

				//std::cout << "Num of MC Particles in this event: " <<  mcparticles.size() << std::endl;
				...
				for (size_t i=0; i<mcparticles.size(); i++) {
					...
					//p_PDG is obtained from the larsoft event (p.PdgCode) 
					//fPdg is obtained from the fcl file (pdg_to_reweight)
					...
					if (p_PDG == fPdg){
						...
						//std::cout << "Num Traj Pts: " <<  p.NumberTrajectoryPoints() << std::endl;
						...
						//moved this outside of istep loop, should now only add daughter particle once
						...
						//if not neutron, use bethe bloch method
						if(p_PDG != 2112){
							thin_slice_inelastic = ThinSliceBetheBloch( &theTraj, .5, mass , false);
							thin_slice_elastic = ThinSliceBetheBloch( &theTraj, .5, mass , true);
						}			
						//if neutron, assume equal amount of edep in each slice in step
						else {
							thin_slice_inelastic = NeutronEnergyDep( &theTraj, .5, mass , false);
							thin_slice_elastic = NeutronEnergyDep( &theTraj, .5, mass , true);
						...
						for( size_t islice = 0; islice < thin_slice_inelastic.size(); ++islice ){
							//		std::cout << thin_slice_inelastic[islice].first << std::endl;          
						...
						for (size_t j=0; j<weight[0].size(); j++) {
							...
							//			std::cout << "Getting weight for this universe" << std::endl;
				...
				//std::cout << "Num of neutrons in this event: " << n_neutrons << std::endl; 
				//std::cout << "Num of protons in this event:  " << n_protons << std::endl;
